...
const AddLesson = () => {
    ...
    setSpinActive(true); 
    fetch("https://behaweb.com/school-admin/add-lesson/",{
      method: 'post',
    ...
    const onDeleteLesson = (lessonId) => {
      ...
      fetch("https://behaweb.com/school-admin/deleteLesson/" + lessonId)
          .then(res => res.json())
          .then(
              (result) => {
                if(result.status === 's'){
    ...
    const updateTeacher = (teacherId,lessonId) => {
      ...
      setSpinActiveTable(true); 
      fetch("https://behaweb.com/school-admin/update-teacher/",{
      method: 'post',
      headers: { 'Content-Type': 'application/json' },
